...
- **E10.2.1 Console header gauges & GO/STOP buttons** · Goal: the two text clock columns become caption + analog `RaceClock` dial + numeric readout (`elapsed_clock`/`remaining_clock` in the XRC placeholder panels `elapsed_clock_panel`/`remaining_clock_panel`; elapsed fills, remaining drains, fractions of planned duration, zeroed in DRAFT), a code-side `StopLight` (`ride_status_light`, green RUNNING · amber DRAFT/REOPENED · red FINISHED — colour never the sole channel) sits beside the status label in the header's leading `ride_status_panel` column (the lamp floored at its own `DoGetBestSize()` so the leading column cannot squeeze it away), and `start_btn`/`stop_btn` become round green GO / red STOP `wxBitmapButton`s whose labels are re-applied in code. Refs: views/gauges.py (RaceClock/StopLight/go_bundle/stop_bundle), main.xrc, views/main_frame.py, presenters/console.py (`stop_light_mode`, `set_clock_fractions`), xrc-windows.md §A, requirements R-83. Tests first: `tests/unit/presenters/test_console.py`, `tests/functional/test_console_live.py`. Done when: dials/light/buttons render and their state mapping is asserted.
