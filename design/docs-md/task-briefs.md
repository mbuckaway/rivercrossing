...
- **E1.3.1 Author the .xrc files** (sessions may split main+setup / riders+detail+results / rest; nine per the §15b map, ten today after E11's `simulation.xrc`) · Goal: all 23 windows exist as sizer-based XRC with frozen names. Refs: XRC canvas — every window block + its code-side footnotes; §15b file map and its measured list of what XRC cannot express; canvas radio defaults (solo, pooled, appearance-System — `<value>` on a wxRadioButton is honoured, unlike `<checked>` on a radio *menu* item; jokers is a dropdown that opens on 1). Tests first: `tests/functional/test_xrc_load.py` — parametrized: each file loads via wx.xrc, every §15b control name resolves, radio defaults as drawn, wxStdDialogButtonSizer present in dialogs; the lists are wxDataViewCtrl, never wxDataViewListCtrl (whose handler forces the name `dataviewCtrl`); the four info bars are absent from the resources by design and `main_menubar` comes from `XmlResource.LoadMenuBar()`, not `FindWindowByName`; window minimums declared as `<size>` and re-applied with `SetMinSize()`; negative: unknown name lookup returns None (asserted for one retired name). Implement: hand-authored .xrc. Done when: 23/23 load with names + defaults on Windows and macOS.
...
- **E1.4.1 Menubar + routes** · Goal: every §15 row reaches its target. Refs: Spec §15 table (38 rows: File 8 · Ride 9 · Riders 4 · Cards 4 · Results 8 · View 1 · Help 4 — the Ride/Cards/Results split moved with the later additions and retirements) + §15b mi_* names; accelerators Enter/Ctrl+Z/F1/F5. Tests first: `tests/functional/test_menu_coverage.py` — walk every row, assert window/dialog/command target; items resolve through the loaded menubar, not FindWindowByName (wxMenuBar drops its name — §15b); radio-item defaults mi_theme_system + mi_zoom_100 are checked in code, since `<checked>` is a no-op on radio items; macOS relocation (About/Settings/Quit in app menu); the three windowless routes (Duplicate Ride…, Reopen Ride, Void Card…) hit a flagged sentinel the walk asserts as a sentinel, not a window *(all three were later retired — Duplicate Ride…/Reopen Ride to Phase 11 H2's native `std_dialogs` confirms, the Void Card… row dropped in Phase 2 — so the walk targets real routes today)*; negative: an unrouted mi_ id fails the walk. Implement: menubar XRC + command table (the accelerator table other features consume). Done when: coverage walk green both OSes.

- **E1.4.2 State enablement** · Goal: menus + buttons obey ride state. Refs: §15 "Enabled when" column; canvas footnotes (DRAFT/FINISHED/REOPENED deltas). Tests first: parametrized (item × state DRAFT/RUNNING/FINISHED/REOPENED) from a table literally transcribed from §15; negative: stop_btn without armed checkbox stays disabled (R-35). **1.0.12 C2 amendment:** the arm-to-stop checkbox is retired — `stop_btn` is enabled by the engine's own live-RUNNING state with no arm gate, so that negative case no longer applies. Implement: mocked state provider + enablement binder. Done when: table test green.
...
- **E1.5.2 Lists demo display** · Goal: library/editor/detail/results show demo data. Refs: #librarydlg #ridereditor #entrydetail #resultsframe rows. Tests first: per-window row/column assertions incl. results publish-checkbox defaults (times ✗, laps ✓, time-board ✗, full ✓, all-cards ✓) *(the five options are now the checkable Results ▸ Publish Options rows — G6 — with those defaults persisted in `AppSettings.publish_*`)* and Team column hidden in solo-only mock. Implement: bindings. Done when: assertions green.

- **E1.5.3 Dialog behavior** · Goal: R-76 mechanics everywhere. Refs: R-76; canvas stock-ID footers. Tests first: per-dialog — Esc cancels, Enter fires default, destructive confirms focus Cancel, focus returns to opener; `message_lbl` in delete_ride_dlg / resume_dlg carries its interpolated line and is asserted non-empty (a blank line is what earned that name; `continue_or_new_dlg` was retired in ux-polish, R-53); negative: Delete in delete_ride_dlg disabled until exact name typed. Implement: wiring only. Done when: parametrized suite green.
...
- **E2.2.2 Shoe config** · Goal: decks × jokers (0–4, dropdown default 1) per ride setup. Refs: #setupdlg Cards fieldset; R-13. Tests first: composition counts per config; jokers=0 never deals a joker (property). Done when: config matrix green.
...
- **E5.2.2 Resume + reopened banners** · Goal: launch with running ride → resume_dlg (crash vs quit wording); the REOPENED banner on the console. Refs: #resumedlg, #mainframe status bar. Tests first: harness relaunch scenarios both wordings — the presenter writes them into `message_lbl` (ride name + quit-vs-crash), and a blank label is a failed assertion, not a cosmetic one; Continue resumes with elapsed correct; Open library path; the REOPENED banner is posted to the status bar's first field (G4) — there is no `reopened_infobar` control, so no banner text can change the frame's height. Done when: both paths green.
...
- **E5.4.1 Library live** · Goal: ride_library_dlg on the real DB (open/new/duplicate — setup+roster only, R-15). **Mock-first step first:** §15's Duplicate Ride… dialog and Reopen Ride confirm have no frozen window (they cite the retired 3d pattern), so this session produces both mockups and registers their control names in §15b before any UI code (plan §2), replacing E1.4.1's sentinel. *(Both were authored mock-first as planned; Phase 11 H2 later retired them to native `std_dialogs.show_prompt` confirms — no XRC window remains.)* Tests first: duplicate copies no timing data; open switches console context; the two new windows load, resolve their names and honour R-76. Done when: green.

- **E5.4.2 Demo retirement** · Goal: remove the DemoDataSource wiring line; demo becomes tests-only. Tests first: lint rule tightened (no bootstrap exemption) red-then-green; full smoke suite still green (screens now show real/empty states). Done when: app paths demo-free; empty-state screenshots attached. *(`rivercrossing.demo` and its import-linter contract were later deleted outright — only the `wx`-under-`ui` contract remains.)*
...
- **E6.4.1 Standings window live** · Goal: results_dlg standings + publish checkboxes → ExportOptions *(G6 moved the five publish options out of the dialog to the checkable Results ▸ Publish Options rows, persisted as `AppSettings.publish_*`, so the dialog carries no checkboxes today)*; stale_infobar constructed in code and named with SetName(), present but hidden (E7 triggers) — XRC cannot author a wxInfoBar (§15b). Refs: #resultsframe. Tests first: harness — checkbox toggles change rendered exports (times case doubles as R-63 UI proof); tie rows badge. Done when: green.
...
- **E7.2.1 Correction dialogs live** · Goal: wire #editcrossing (both titles), #reassigndlg, #dealdlg, #dnfdlg + entry-detail buttons to E7.1/E4.3.2 paths. *(Shipped differently: there is no reassign dialog or `entry_detail` view — reassign and void run from Crossing Detail (`views/crossing_detail.py`, `crossing_detail_dlg`), which owns those corrections; the Edit Crossing menu row was retired in G7.)* **Mock-first step first:** the Void Card… confirm has no frozen window (§15 cites the retired 3d pattern), so mock it and register its names in §15b before wiring, replacing E1.4.1's sentinel. *(The `void_card_confirm_dlg` was authored as planned; the Cards Void Card… row was later dropped in Phase 2.)* Tests first: harness per dialog incl. edit-vs-add prefill, DNF reversible, void-card from cards row. Done when: every §15 Cards-menu route exercises its command.
...
- **E9.1.2 Windows installer** · Goal: NSIS per-user .exe, unsigned v1 (Spec §10/§14; Phase 9 amendment: NSIS replaces Inno Setup). *Phase 9 (EPIC 1 follow-up) already delivered the unsigned half: `installers/windows.nsi` (per-user, Start-menu entry, HKCU uninstall key, branded icon, no hard-coded version), `nox -s winsetup` / `winsetup_smoke` (native makensis compile smoke on macOS), and the windows-latest packaging job (blocking since Phase 10) that compiles the setup .exe, runs the silent install/launch/uninstall suite and uploads the artifacts. What remains here: Authenticode signing only — Phase 11 delivered the tagged-release wiring (a version tag publishes both unsigned installers with checksums).* Tests first: silent install/uninstall on windows-latest leaves/removes files + Start-menu entry. Done when: install-run-uninstall green.
...
### E10 · Console & setup polish — ux-polish (retrospective, shipped 2026-09-06 on `topic/ux-polish`)

Retrospective briefs for the ux-polish wave (new requirements R-79…84). Work already landed; these record what shipped for traceability. Mock anchors: the console header gauges (WS-D) and the two-tab review notebook (WS-H) are drawn in xrc-windows.md section A; `no_ride_dlg` in section B; the reworked Teams Editor and the About box in sections C/E.

- **E10.1.1 Start gate + minimum-setup rule** · Goal: a DRAFT ride's Start refuses an empty roster ("roster has no riders") or a setup missing a minimum field (blank name/venue/organizer/scorer or a non-positive lap length — `ride.setup_minimum_violations`, reasons joined, `StartBlockedError`); Ride Setup refuses the same incomplete submit on `setup_infobar`, and the retired continue-vs-new flow (`continue_or_new_dlg`) is gone — starting a stopped ride with data continues directly. Refs: `ride.py` (`setup_minimum_violations`, `RideEngine.start`, `StartBlockedError`), `presenters/setup.py`, `views/ride_setup.py`, dialogs.xrc, xrc-windows.md §B, requirements R-79. Tests first: start-gate refusals in `tests/unit/test_ride.py`, submit refusals in `tests/unit/presenters/test_setup.py`, functional coverage in `tests/functional/test_ride_setup.py` and `test_main_frame_guard.py`. Done when: the gate tests are green and no src reference to `continue_or_new_dlg` remains.
- **E10.1.2 Startup no-ride prompt** · Goal: a store-backed launch that resumes no ride shows `no_ride_dlg` once the console is wired — Create new ride… (primary, Enter) opens the ride-setup flow, Open library… (secondary, Escape) the library — and a launch that already deferred Open library from `resume_dlg` (`library_deferred`) never double-prompts. Refs: dialogs.xrc (`no_ride_dlg`), `ui/app.py` (`_show_no_ride_prompt`), xrc-windows.md §B, requirements R-80. *(`no_ride_dlg` was retired in W15 and its native launch-alert replacement removed 2026-09-13 — R-80 retired; a no-candidate launch now shows nothing, and creating or loading a ride is the menus' own job.)* Tests first: `tests/functional/test_app_bootstrap.py`. Done when: prompt, both button routes and the no-double-prompt launch are green.
- **E10.2.1 Console header gauges & GO/STOP buttons** · Goal: the two text clock columns become caption + analog `RaceClock` dial + numeric readout (`elapsed_clock`/`remaining_clock` in the XRC placeholder panels `elapsed_clock_panel`/`remaining_clock_panel`; elapsed fills, remaining drains, fractions of planned duration, zeroed in DRAFT), a code-side `StopLight` (`ride_status_light`, green live RUNNING · amber REOPENED/stopped RUNNING · red DRAFT/FINISHED — colour never the sole channel) sits beside the status label in the header's fixed-width "Status" group box (`ride_status_panel`, far-left in the header; the lamp floored at its own `DoGetBestSize()` so the fixed-width box cannot squeeze it away), with a new untitled group box holding `current_lap_lbl` beside it, and `start_btn`/`stop_btn` become round green GO / red STOP `wxBitmapButton`s whose labels are re-applied in code. Refs: views/gauges.py (RaceClock/StopLight/go_bundle/stop_bundle), main.xrc, views/main_frame.py, presenters/console.py (`stop_light_mode`, `set_clock_fractions`), xrc-windows.md §A, requirements R-83. Tests first: `tests/unit/presenters/test_console.py`, `tests/functional/test_console_live.py`. Done when: dials/light/buttons render and their state mapping is asserted.
- **E10.2.2 Review notebook + rider open seam** · Goal: the review sidebar becomes the two-tab `review_notebook` — Needs Review (`flagged_list` Plate | Lap | Lap time + `review_btn`, the page Cards ▸ Review Held Cards focuses) and Riders (`console_riders_list` Plate | Name | Team over the live roster, refreshed on the tick) — and activating a rider row (double-click or Enter) opens `rider_editor_dlg` pre-selected at that rider's plate. Refs: main.xrc, views/main_frame.py (`set_on_open_rider`), ui/app.py (`_wire_rider_open_seam`, `_open_rider_editor_for`), xrc-windows.md §A, requirements R-84. Tests first: `tests/functional/test_console_live.py`. Done when: both tabs, the review route and the pre-selected editor open are asserted.
- **E10.3.1 Editors & setup defaults** · Goal: ride setup defaults to teams — `mixed_radio` "Solo + teams (default)" is the XRC `<value>1</value>` and the store-backed bootstrap's fresh roster opens MIXED/rider-pooled — the rider editor's `team_choice` drops the "New team…" sentinel (solo or an existing team only), and the Teams Editor is reworked: Team | Riders | Logo columns (Logo cell "Card"/"Image"), a real-bitmap `logo_bmp` preview above the Members box, multi-line Notes, ~50/50 panes, Save/Remove/Close bottom-left + Add team bottom-right, and Add creates from the form with blank/duplicate-name rejection (trimmed, case-insensitive — renames on Save too). Refs: setup.xrc/teams.xrc/riders.xrc, views/team_editor.py, presenters/teams.py (`on_add`), presenters/riders.py (`SOLO_TEAM_CHOICE`), xrc-windows.md §B/§C, requirements R-81. Tests first: `tests/functional/test_team_editor.py`, `test_rider_editor.py`, `test_ride_setup.py`; `tests/unit/presenters/test_teams.py`, `test_riders.py`. Done when: the rework and both refusal paths are green.
- **E10.4.1 About box attribution + fixed size** · Goal: `about_dlg` shows the product name + package version, a never-blank logo (ride logo, else the embedded RiverCrossing logo), the fixed attribution lines (Mark Buckaway · © 2026 Mark Buckaway · Licensed under GPL-3.0-only) and the gorba.ca link, fixed-size as the one dialog that drops wxRESIZE_BORDER (ABOUT_SIZE pinned code-side). Refs: dialogs.xrc, views/about.py, xrc-windows.md §E, requirements R-82. Tests first: `tests/functional/test_about_dialog.py`. Done when: attribution lines, logo fallback and the fixed size are asserted.
