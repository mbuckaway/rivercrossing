...
- **E1.2.1 ids.py generator** · Goal: XRC names are the single registry, generated never typed. Refs: Spec §15b naming rules; R-05. Tests first: `tests/unit/test_ids_gen.py` — fixture .xrc → constants (PLATE_INPUT="plate_input"); duplicate name in one window fails; drift (constant missing vs xrc) fails; suffix-convention warnings. Implement: `tools/gen_ids.py` + CI step. Done when: regen is idempotent and drift fails the build.
...
- **E1.3.1 Author the 9 .xrc files** (sessions may split main+setup / riders+detail+results / rest) · Goal: all 23 windows exist as sizer-based XRC with frozen names. Refs: XRC canvas — every window block + its code-side footnotes; §15b file map; canvas radio defaults (solo, pooled, jokers-2, appearance-System). Tests first: `tests/functional/test_xrc_load.py` — parametrized: each file loads via wx.xrc, every §15b name resolves, radio defaults as drawn, wxStdDialogButtonSizer present in dialogs; negative: unknown name lookup returns None (asserted for one retired name). Implement: hand-authored .xrc. Done when: 23/23 load with names + defaults on Windows and macOS.
...
- **E1.3.3 Per-screen smoke** · Goal: the forever-test — every screen loads, shows, closes. Refs: plan §4. Tests first: parametrized over all 23 (Show → names resolve → screenshot artifact → Close returns). Implement: window loader helpers until green. Done when: green on macOS locally + both OSes in CI.

- **E1.4.1 Menubar + routes** · Goal: every §15 row reaches its target. Refs: Spec §15 table (38 rows: File 8 · Ride 7 · Riders 4 · Cards 7 · Results 7 · View 1 · Help 4) + §15b mi_* names; accelerators Enter/Ctrl+Z/F1/F5. Tests first: `tests/functional/test_menu_coverage.py` — walk every row, assert window/dialog/command target; macOS relocation (About/Settings/Quit in app menu); negative: an unrouted mi_ id fails the walk. Implement: menubar XRC + command table (the accelerator table other features consume). Done when: coverage walk green both OSes.
...
- **E5.2.1 + E5.2.3 Session bookkeeping + exit flow** · Goal: clean-quit vs crash recorded; exit-with-running-ride dialog (R-51/52). Refs: #exitdlg. Tests first: unclean flag set on open, cleared on clean close; exit dialog wording + quit keeps ride timing (fake clock across process restart). Done when: green.

- **E5.2.2 Resume + reopened banners** · Goal: launch with running ride → resume_dlg (crash vs quit wording); reopened_infobar on console. Refs: #resumedlg, #mainframe InfoBar. Tests first: harness relaunch scenarios both wordings; Continue resumes with elapsed correct; Open library path. Done when: both paths green.
...
- **E5.3.2 Delete guard** · Goal: R-18 exactly. Refs: #deletedlg. Tests first: harness — Delete disabled until exact name; backup written before delete; RUNNING ride: item disabled. Done when: green.

- **E5.4.1 Library live** · Goal: ride_library_dlg on the real DB (open/new/duplicate — setup+roster only, R-15). Tests first: duplicate copies no timing data; open switches console context. Done when: green.
...
- **E6.4.1 Results window live** · Goal: results_frame standings + publish checkboxes → ExportOptions; stale_infobar present (hidden — E7 triggers). Refs: #resultsframe. Tests first: harness — checkbox toggles change rendered exports (times case doubles as R-63 UI proof); tie rows badge. Done when: green.
...
- **E7.2.1 Correction dialogs live** · Goal: wire #editcrossing (both titles), #reassigndlg, #dealdlg, #dnfdlg + entry-detail buttons to E7.1/E4.3.2 paths. Tests first: harness per dialog incl. edit-vs-add prefill, DNF reversible, void-card from cards row. Done when: every §15 Cards-menu route exercises its command.
...
- **E8.1.2 Appearance** · Goal: R-03 exactly — macOS follows OS; Windows Dark disabled + "needs wxPython 4.3" hint; SetAppearance behind capability check. Tests first: capability fake (has/hasn't SetAppearance) drives radio enablement + application; negative: selecting Dark on capable build applies without restart. Done when: matrix green per-OS.
...
- **E9.1.1 Release bundles** · Goal: harden the E1.6.1 dev-bundle specs into release bundles — full assets/templates/guide/WAVs, version metadata, icons. Tests first: packaged-app smoke (launch, open ride, one crossing, export HTML) driven via the harness against the bundle; asset-manifest completeness vs source tree. Done when: smoke green on clean CI images.

- **E9.1.2 Windows installer** · Goal: Inno Setup per-user .exe, unsigned v1 (Spec §10/§14). Tests first: silent install/uninstall on windows-latest leaves/removes files + Start-menu entry. Done when: install-run-uninstall green.

- **E9.1.3 macOS dmg + notarization** · Goal: dmgbuild, Developer ID codesign + notarize. *Owner: org supplies APPLE creds (contract E1.1.2); until then stage emits unsigned dmg and the notarize gate is advisory.* Tests first: spctl assessment (skipped-with-reason when creds absent); dmg mounts + app launches. Done when: signed path green once creds land; unsigned path green meanwhile.
...
| TB-8 | Mock-first rule check: all 23 windows have frozen mockups (XRC canvas), incl. shortcuts_dlg added in the last audit — no gaps. | No mock-first tasks needed; rule stays in force for any new screen. |
