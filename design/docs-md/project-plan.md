...
| E9 | Packaging & release — installers, signing/notarization, acceptance race | v1.0 | all · external creds |
...
- **Mock-first rule:** the 23 canvas screens have frozen mockups, but §15 routed three items at a dialog the canvas never drew — Duplicate Ride… and Reopen Ride (E5), the Void Card… confirm (E7). Those, and any future screen without a mockup, get a mock-first step that produces the mockup and registers its control names in §15b *before* any UI code. *(All three were authored mock-first as planned, then retired to native `std_dialogs` confirms — Duplicate Ride…/Reopen Ride in Phase 11 H2, the Void Card… route in Phase 2 — so no XRC window for them remains.)*
...
· **E1.4.1** Menubar per §15/§15b incl. Results menu, accelerators (Enter, Ctrl+Z, F1, F5), macOS relocation — S1: menu coverage test walks every §15 row to its target (red) · S2 wire routes. The three rows with no frozen window — Duplicate Ride…, Reopen Ride, Void Card… (§15) — routed to a flagged sentinel the coverage test asserts as such; E5 and E7 replaced it mock-first, and no window name was invented here. *(All three were later retired — Duplicate Ride…/Reopen Ride to Phase 11 H2's native `std_dialogs.show_prompt`, the Void Card… row dropped in Phase 2 for Crossing Detail — so no sentinel remains.)*
...
· **E1.5.3** Dialog behaviors R-76: Esc cancels, Enter default, destructive confirms focus Cancel, arm-to-stop checkbox gates stop_btn *(the arm-to-stop checkbox is retired — 1.0.12 C2; `stop_btn` is enabled by the engine's own live-RUNNING state)* — S1 per-dialog tests · S2 wiring.
...
- **E2.2 Shoe** — E2.2.1 seeded Fisher-Yates, deal_index, exhaustion reshuffle cycle (S1 determinism/exhaustion tests · S2); E2.2.2 decks × jokers config per setup dialog (0–4, dropdown default 1).
...
- **E4.4 Console live** — E4.4.1 feed (newest-first 30), counters, last_crossing_lbl, review panel from real engine; E4.4.2 arm_stop_chk gate + stop/finish confirm flows (1.0.12 C2 amendment: the Arm checkbox and its auto-clear are retired — R-35's Stop is one act, enabled by the engine's own live-RUNNING state and still confirming natively); E4.4.3 sound cues per §10 (recorded/flagged/error) behind Settings toggle; E4.4.4 mini acceptance: scripted 20-rider, min-lap-lowered race in memory through the real UI.
...
- **E5.2 Sessions** — E5.2.1 session_state records clean/unclean close + running ride; E5.2.2 resume_dlg (crash vs quit wording) + the REOPENED banner (status-bar text, G4 — no InfoBar control); E5.2.3 exit_running_dlg flow (quit keeps ride timing on wall clock).

- **E5.3 Backups** — E5.3.1 open + hourly + manual (its one surface is File ▸ Back Up Database…, `mi_backup_now` — the Settings dialog button was removed), keep 20 (R-54); E5.3.2 delete-ride guard: backup first, type-name confirm, never RUNNING (R-18).
...
Shipped in v0.5. The event-sourced store found its home in `rivercrossing.store`: a multi-ride SQLite database (spec §2's full schema; WAL · synchronous=NORMAL · foreign_keys=ON; one flattened v1 baseline with a version ledger — the numbered migration chain was removed in Phase 2, `store/migrations.py` is gone, migrations return after release) whose `audit` table is the event log — `Store.append` persists every engine `Event`, `Store.load_engine` replays it into an identical `RideEngine` (the `apply` seam on the engine + the equivalence property: random seeded sequences, live == replayed; the replay work also fixed an E4 defect — undo after `deal_manual`). R-50 is proven by a 50-kill crash-consistency loop (subprocess killed at seeded points → reopen: `integrity_check` ok, exactly a committed prefix). Sessions: `app_session` bookkeeping (`SessionState` CLEAN_QUIT/CRASHED/RUNNING_AT_EXIT, read from the second-newest row), the three-button exit-with-running-ride dialog (Cancel default · finish first · quit-keep-running, `closed_at` stamped), the resume dialog with crash-vs-quit wording, and the reopened banner. Backups: `backup.run` (WAL-checkpointed copy into `<db>.backups/`, rotation to keep 20), `schedule_hourly` seam, `restore`; the R-18 delete guard (type-name confirm, backup-first, never RUNNING). The library is live on the real DB (open/new/duplicate/delete; roster persisted to the `entry`/`rider` tables; `duplicate_ride` copies setup + roster with no timing data). **The two windowless §15 routes were authored mock-first:** Duplicate Ride… and Reopen Ride confirms, their control names registered in §15b before any UI code, replacing E1.4.1's sentinel — *both later retired to Phase 11 H2's native `std_dialogs.show_prompt` confirms.* The demo seam is retired from the app path (E5.4.2): the import-linter contract that kept `rivercrossing.demo` to tests *(since deleted outright with the module — only the `wx`-stays-under-`rivercrossing.ui` contract remains)*, every screen shows real or a documented empty state, and empty-state screenshots are captured. The wx⇄asyncio integration decision is recorded in §7 (E5 keeps the writer synchronous; `wxasync` stays ruled out — see the risk row).
...
- **E6.1 Standings** — the core (rank, R-14 tie-break order, R-43 draw-required, DNF exclusion, both leaderboards) shipped in E4 (v0.4, `rivercrossing.standings`); E6 adds what E4 deliberately deferred: the human-readable hand-name renderer, the results-window live standings wiring, and the draggable tie-break reorder UI. (The R-14 high-card draw itself is the engine's: `RideEngine.finish()` deals one card per tied entry from one fresh 52-card deck seeded from the ride's stored seed, records the audited `tiebreak_draw` event, and `standings` orders a fully drawn group by `cards.draw_key`.) E6.1.1 crafted tie fixtures / reorder re-runs / per-rider DNF excluded from the ranking — no DNF block (S1 · S2); E6.1.2 leaderboards already live from E4.
...
- **E6.4 Standings window live** — E6.4.1 results_dlg standings + publish options driving ExportOptions *(G6 moved the five options out of the dialog to the checkable Results ▸ Publish Options rows, persisted as `AppSettings.publish_*`)*; E6.4.2 Results menu items (Export HTML / Export PDF / Poster / Standings CSV / Preview HTML in Browser / Preview PDF in Browser) with FINISHED gating; E6.4.3 finish gate: the evaluator self-test is consulted at the finish — only a failed BLOCKING check gates it (the whole-field 180×12 timing check is advisory, it times the host) and the operator may override with "Finish anyway", which records the failed checks on the finish event and marks the results self-test unverified (§2, R-44).
...
Shipped in v0.6. The prose hand-name renderer (`standings.hand_name`, title-case em-dash — one style for window and exports) and the stored tie-break spelling ⇄ `TieBreak` conversion landed in `rivercrossing.standings` (E6.1.1), with E6.1.2's leaderboard negative guard pinned as a test. The HTML export is real: the frozen templates ship verbatim, `tools/gen_css.py` vendors the Tailwind v4 output (`compiled_css`) and base64 Barlow subsets (`fonts_css`) with a `css_drift` staleness gate in CI, and `htmlexport.render()` (autoescape + StrictUndefined + the `racejson` `</`-escaping filter) reproduces the committed golden pages byte-for-byte — the three samples' `race-data` blocks are the fixtures, the goldens were regenerated deliberately once (TB-5) and frozen, and the no-times variant omits time markup AND JSON (R-63). `rivercrossing.pdfexport` renders the multi-section report and the one-page podium poster with byte-deterministic output across OSes (timezone-aware-UTC creation date, embedded OFL Barlow/Barlow Condensed/DejaVu fonts — D14). The Standings window is live (E6.4.1): real placed rows from the console's engine source, the tie-break order from Ride Setup's `tiebreak_list` (the Results dialog carries no tie-break control — `mi_tiebreak_order` was retired), the five publish options on the Results ▸ Publish Options rows (G6) driving `ExportOptions` (show-times hides the Total column too), a ⚠ badge marks draw-required ties, and `stale_infobar` waits hidden for E7. The Results menu's five export rows and Preview HTML in Browser / Preview PDF in Browser are wired (E6.4.2; the Tie-break Order row was later retired) with off-loop writes (R-02); `csvio.export_standings` ships the §15 standings CSV and `csvio.export` gained the §7 finished-ride columns (approved addition). The finish gate (E6.4.3) runs the evaluator self-test fresh; a later revision splits the six checks into five blocking correctness checks and one advisory timing check (which never gates a finish), so a blocking failure now raises a second confirm naming every failed check — "Finish anyway" records them on the finish event (`self_test_failed_checks`) and marks the results self-test unverified, while Cancel leaves the ride unfinished (R-44). **The task-briefs "one task per PR" rule was overridden for this EPIC by product decision: all twelve task commits land in the single EPIC PR (#12), in test-first order.** The known `wx.adv.EditableListBox` seam (generic New/Delete buttons nothing suppresses) is validated-and-restored by the results presenter, matching ride_setup's precedent.

E7 · Corrections & audit

Every scorer's-table mistake is fixable with a reason and a trail: edit/void crossings, add-at-time, reassign plates, manual deal, void card, DNF, reopen-for-corrections, and the filterable audit viewer. The **Void Card…** confirm was the third §15 route with no frozen window: E7 authored it mock-first per §2 and registered its names in §15b. *(Phase 2 later dropped the Cards Void Card… row — Crossing Detail owns the void — leaving no §15 route on it.)*
...
- **E7.2 Dialogs live** — E7.2.1 edit_crossing (edit/add-at-time titles), reassign, manual_deal, dnf_confirm, void flows from entry_detail *(the reassign and void flows now run from Crossing Detail, `crossing_detail_dlg`; the entry_detail view and the reassign dialog were retired in Phase 2, and the Edit Crossing menu row in G7)*; E7.2.2 REOPENED mode: entry off, corrections on, edited rows highlighted, "Finish again".
...
Shipped in v0.7. The audited command layer landed on `RideEngine` (E7.1): `edit_crossing`, `void_crossing`, `add_crossing_at`, `reassign_crossing`, `mark_dnf`, and `void_card` — each requiring a non-empty reason and writing exactly one audit event — `undo_last` now carries the fixed label "Undo last crossing", every new action replays through `apply()` (with `Shoe.is_closed` now part of the replay-equivalence contract), and `snapshot()` marks DNF entries so `standings.rank` excludes them (per-rider DNF; E7.1.2's "history + corrections replayed == directly corrected history" property green). The correction UI is live (E7.2): Crossing Detail's commands and the §15 Cards-menu routes drive the real commands through a new `views/corrections` runner (`entry_detail`'s view and `DetailPresenter` were retired in Phase 2; the corrections run from `views/crossing_detail.py`), and `ui/menu_state.py` applies `is_route_enabled` at runtime — the missing E1.4.2 live binder. REOPENED is corrections-only (plate entry disabled, corrected crossings bolded in the feed, the banner posted as status-bar text — no `reopened_infobar` control — single "Finish again" that re-locks and re-ranks). The audit viewer (E7.3.1) is `Store.audit_rows` (newest-first, projected `when/who/action/entry/reason`) plus `AuditPresenter` and `views/audit.py` with plate-or-entry-name search, the six §15 action buckets, and entry-detail deep-linking; the stale-export flag (E7.3.2) marks published results stale after a post-export correction and clears on re-export (watermark = `len(engine.events)`, `None` = never exported). The third mock-first window — `void_card_confirm_dlg` (voids a dealt card) — was authored and its names registered in §15b, retiring the last `_UNAUTHORED_DIALOG` sentinel. **Shoe reopen (product decision):** `finish()` still closes the shoe, but `reopen()` now re-opens it so `deal_manual` and `add_crossing_at` keep dealing in REOPENED, resolving the §4-vs-§15 tension. Doc-silence rulings written back: `reassign_crossing`'s `seq` is the ride-wide ordinal into `engine.crossings`; correction menu routes target the current entry (tracked as `detail_plate` — the dialogs carry no plate/crossing selector); `add_crossing_at` credits directly (never holds); "edited" means a crossing touched by any correction event; `Store.audit_rows` returns the `AuditRow` view-model (the core→presenters seam the retired `demo.py` established); the move-rider picker is built in code (no XRC window exists).
...
Shipped in v0.8. Settings persist per-user in a `platformdirs.user_config_dir` JSON file (`ui/presenters.settings`: `AppSettings` grows `splitter_sash`/`window_geometry`; `default_path`/`load_settings`/`save_settings` as plain module functions; missing/corrupt files fall back to defaults, zoom clamps onto the 90–150 ladder) — the product decision to keep app preferences out of the ride DB, with the two stale `settings`-table notes in `store/` corrected. The bootstrap loads settings, applies appearance/sound/hide-times, restores sash + geometry, and persists on change/close; `MainFrame`'s process-lifetime sash global is gone. The settings dialog (`views/settings.py`) renders and collects everything, and the appearance System/Light/Dark radios drive the same `ThemeController` as the View menu, mirrored both ways and persisted (R-03); `theme.apply()` gained the capability guard. Hide-times toggles the console columns live mid-ride (View menu ↔ Settings, persisted). Zoom is a new `ui/zoom.py` (`ZoomController`, recursive base-font capture so 90–150% never compounds) wired to the View radios and the settings choice, applied at open to every dialog (R-04). The shortcuts dialog (`views/shortcuts.py`) generates its rows from `ui.accelerators.ACCELERATOR_TABLE`, generativity-proven. The user guide is a complete self-contained `docs/user-guide.html` (10 chapters + 2 appendices per the 6a outline, light/dark-aware, per-section anchors); `ui/help.py` maps every window to its anchor, and F1 / Help ▸ User Guide opens the guide deep-linked to the active window (default anchor otherwise). The About box (`views/about.py`) shows the package version, the gorba.ca link, and the ride logo with an app-icon fallback chain. Scenario hermeticity: every functional app launch uses a per-scenario tmp settings path, and the Tart VM functional gate runs `--no-audio`. Windows-only observation handed to the Windows agent (`docs/WINDOWS-AGENT-HANDOFF.md`): `main_frame` restores pinned to its sizer minimum on Windows (1100×788 — larger than the 1024×720 work area), the signature of a frame without `wxRESIZE_BORDER`; the E8 geometry test asserts the round-trip equivalently rather than a hardcoded size.

E9 · Packaging & release — external creds live here, last

Installables and the full-dress rehearsal: PyInstaller apps, NSIS .exe (per-user, unsigned — Phase 9: NSIS replaces Inno Setup) and notarized .dmg, CI stages 5–6, the complete acceptance race, and the seeded run.
...
- **E9.2 Acceptance** — E9.2.1 full race R-74: CSV in, hundreds of typed crossings, stop/continue, kill+relaunch, quit+relaunch, finish, all four exports verified; E9.2.3 release drafting on version tags.

Exit criteria installers install and run on clean Windows 11 + macOS images · acceptance green · v1.0 tagged with artifacts attached.

Shipped in v1.0. The full-dress rehearsal is wired, and the Phase 0 spike surfaced (and EPIC 9 closed) the biggest latent gap in the codebase: **the production UI had no write path to the store** — `main()` launched an empty console with `store=None`, and no production code called `Store.open`/`create_ride`/`save_roster`/`append`, so the app could not open, create, or persist a ride despite E5's store being complete and tested. Phase 1 wired the store-backed app layer: `main()` resolves `platformdirs.user_data_dir("RiverCrossing")/rides.db` and opens the Store (`RIVERCROSSING_DB_PATH` overrides it for tests), resume-on-launch fires, New Ride persists via `create_ride`+`save_roster` and switches the console onto the new ride, and every engine event persists via `Store.append` (an `on_event` sink attached after replay, so the replayed tail is never re-appended); CSV import persists the imported roster to the active store ride. E9.1.1 ships the user guide and the GPL/OFL license texts in the bundle (`rivercrossing/docs/`, a fifth asset-manifest tree verified at build time) and extends the packaged-app smoke toward launch → open ride → crossing → HTML export. E9.1.2 remains unsigned by decision (R-01, no Windows cert) with the install-run-uninstall coverage green on windows-latest. E9.1.3 delivers the signed-lane contract — the tag `release` job in `.github/workflows/ci.yml` codesigns, notarizes, staples and `spctl`-assesses on tags when `APPLE_*` secrets are present, publishing all three arch-tagged installers, and stays advisory (unsigned DMG) until the org supplies the E1.1.2 credentials; `test_release_signing.py` skips-with-reason meanwhile, and the `BUNDLE_ID` confirmation TODO is recorded in the spec. E9.2.1 is the release gate: `tests/acceptance/` proves the full R-74 race end-to-end — CSV in, hundreds of typed crossings, stop/continue, kill+relaunch and quit+relaunch against one shared `rides.db` (a multi-process child seam), finish, reopen → add-at-time + void → finish again, and all four exports parsed and asserted — and stage 4 joins the CI matrix and the tag run. E9.2.3 cuts v1.0.0. **Open items:** Apple signing creds (external), `BUNDLE_ID` confirmation against the developer account, the pre-Tahoe DMG-background verification, and — resolved 2026-09-01 — the signed lane: Developer ID signing + notarization now live in ci.yml's tag `release` job (GitHub's cross-workflow `needs`/artifact limits made a separate release.yml lane impossible); `release.yml` was retired.

E10 · Console & setup polish — ux-polish (retrospective, shipped 2026-09-06)

A post-E9 polish wave on `topic/ux-polish` (new requirements R-79…84; briefs in [task-briefs.md](task-briefs.md) §E10). Ride start is gated on a minimum setup and a non-empty roster, and the continue-vs-new prompt (`continue_or_new_dlg`) is retired — a stopped ride with data continues directly; a ride-less store launch now asks Create new ride / Open library instead of showing an unexplained empty console (`no_ride_dlg`). Ride setup defaults to **teams** (`mixed_radio` "Solo + teams (default)"; the fresh-roster bootstrap opens MIXED/rider-pooled), the console header gains analog `RaceClock` dials with numeric readouts, a `StopLight` status lamp and round GO/STOP bitmap buttons, and the review sidebar becomes the two-tab notebook (Needs Review / Riders, whose rows double-click into the Rider Editor pre-selected). The Teams Editor reworks to three columns, a real `logo_bmp` preview, multi-line notes and add-from-form with duplicate-name rejection; the rider editor's "New team…" sentinel is gone (teams are created only in the Teams Editor); the About box gains fixed attribution and a pinned size.

- **E10.1 Ride start & bootstrap** — E10.1.1 start gate + minimum-setup rule (empty-roster and missing-field refusals; setup dialog refuses the same submit); E10.1.2 no-ride startup prompt (`no_ride_dlg`, no double-prompt after a resume-to-library launch).
- **E10.2 Console** — E10.2.1 header gauges and GO/STOP bitmap buttons (RaceClock dials + StopLight, both code-side into the XRC placeholder panels); E10.2.2 review notebook's two tabs and the rider-row → Rider Editor open seam.
- **E10.3 Editors & setup defaults** — E10.3.1 teams-first setup default, rider-editor team_choice without "New team…", and the Teams Editor rework (Team | Riders | Logo columns, `logo_bmp` preview, multi-line Notes, relocated buttons, add-from-form with blank/duplicate-name rejection).
- **E10.4 About** — E10.4.1 fixed-size About box with attribution lines and the never-blank embedded-logo fallback.
...
| E9 | R-01, R-74/75 · Spec §10 installers, §14 stages 5–6 | — |
| E10 (ux-polish) | R-79…84 · supersedes R-53's continue-vs-new prompt and flips the entry-mode default (teams) that R-11/§1 recorded as solo | main_frame (gauges + review notebook), ride_setup_dlg, team_editor_dlg, no_ride_dlg, about_dlg · Spec §3/§6/§13/§15b |
...
| wx⇄asyncio integration undecided | **Resolved (EPIC 5, 2026-08-28)** — `wxasync` is out (Spec §10 — one teardown path segfaults, another hangs, and a segfault on quit would forge a crash signal against R-52). E5 kept the sqlite3 writer **synchronous**: every `Store.append` is one fast WAL transaction, and the 50-kill crash-consistency loop proves the durability story, so no E5 path needs the UI loop offloaded for writes. When a genuinely async writer is needed (E6 exports/imports progress reporting), the mechanism is **`wx.CallAfter` + a background thread** (the repo's established quit-flow deferral idiom), not an asyncio event loop. |
...
- **Launch-flow reorder** (W3): the normal launch presents no modal before `frame.Show()`. `build_main_window` shows nothing; `main()` shows the frame, then runs the post-Show `_run_launch_flow` (resume dialog, else nothing at all — the native No Ride Open alert it once showed was removed 2026-09-13, R-80 retired). A modal that cannot present before Show previously blocked the launch invisibly.
...
- **Every plate is a whole number** (W7 amended): the csvio-vs-roster docstring contradiction resolved in favour of NUMERIC — a plate is a whole-number string under every plate model (`rider_pooled` and `team_relay`), and a blank or non-numeric plate is refused by every plate entry point (`_require_plate_nonempty` then `_require_whole_plate`); csvio's module docstring records the same resolution and a non-digit `NUMBER` cell is a per-row conflict.
...
- **Native standard dialogs** (W2/W15/H2, R-85): `ui.std_dialogs` (`show_info`/`show_warning`/`show_error`/`show_confirm`, joined by `show_danger` and `show_prompt` in H2) replaces bespoke XRC windows for one-shot alerts and destructive confirms; `stop_confirm_dlg` retired in W5, `no_ride_dlg` in W15 (its replacement launch alert was itself removed 2026-09-13 — R-80 retired, the no-candidate launch now shows nothing), and `finish_confirm_dlg`, `exit_confirm_dlg`, `duplicate_ride_dlg` and `reopen_ride_dlg` in H2 — leaving the 200-name registry of §15b.
