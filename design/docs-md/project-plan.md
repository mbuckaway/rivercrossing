...
- **Every plate is a whole number** (W7 amended): the csvio-vs-roster docstring contradiction resolved in favour of NUMERIC — a plate is a whole-number string under every plate model (`rider_pooled` and `team_relay`), and a blank or non-numeric plate is refused by every plate entry point (`_require_plate_nonempty` then `_require_whole_plate`); csvio's module docstring records the same resolution and a non-digit `PLATE` cell is a per-row conflict.
