...
Shipped in v0.6. The prose hand-name renderer (`standings.hand_name`, title-case em-dash — one style for window and exports) and the stored tie-break spelling ⇄ `TieBreak` conversion landed in `rivercrossing.standings` (E6.1.1), with E6.1.2's leaderboard negative guard pinned as a test. The HTML export is real: the frozen templates ship verbatim, `tools/gen_css.py` vendors the Tailwind v4 output (`compiled_css`) and base64 Barlow subsets (`fonts_css`) with a `css_drift` staleness gate in CI, and `htmlexport.render()` (autoescape + StrictUndefined + the `racejson` `</`-escaping filter) reproduces the committed golden pages byte-for-byte — the two samples' `race-data` blocks are the fixtures, the goldens were regenerated deliberately once (TB-5) and frozen, and the no-times variant omits time markup AND JSON (R-63). `rivercrossing.pdfexport` renders the multi-section report and the one-page podium poster with byte-deterministic output across OSes (timezone-aware-UTC creation date, embedded OFL Barlow/Barlow Condensed/DejaVu fonts — D14). The results window is live (E6.4.1): real placed rows from the console's engine source, the tie-break order re-ranks through the control's native arrows, publish checkboxes drive `ExportOptions` (show-times hides the Total column too), a ⚠ badge marks draw-required ties, and `stale_infobar` waits hidden for E7. The Results menu's five export rows, Preview in Browser and Tie-break Order are wired (E6.4.2) with off-loop writes (R-02) and the window's own export buttons route through the same handlers; `csvio.export_standings` ships the §15 standings CSV and `csvio.export` gained the §7 finished-ride columns (approved addition). The finish gate (E6.4.3) now runs the evaluator self-test fresh; a red suite blocks finishing with a notice (R-44). **The task-briefs "one task per PR" rule was overridden for this EPIC by product decision: all twelve task commits land in the single EPIC PR (#12), in test-first order.** The known `wx.adv.EditableListBox` seam (generic New/Delete buttons nothing suppresses) is validated-and-restored by the results presenter, matching ride_setup's precedent.
