...
Shipped in v0.6. The prose hand-name renderer (`standings.hand_name`, title-case em-dash — one style for window and exports) and the stored tie-break spelling ⇄ `TieBreak` conversion landed in `rivercrossing.standings` (E6.1.1), with E6.1.2's leaderboard negative guard pinned as a test. The HTML export is real: the frozen templates ship verbatim, `tools/gen_css.py` vendors the Tailwind v4 output (`compiled_css`) and base64 Barlow subsets (`fonts_css`) with a `css_drift` staleness gate in CI, and `htmlexport.render()` (autoescape + StrictUndefined + the `racejson` `</`-escaping filter) reproduces the committed golden pages byte-for-byte — the three samples' `race-data` blocks are the fixtures, the goldens were regenerated deliberately once (TB-5) and frozen, and the no-times variant omits time markup AND JSON (R-63). `rivercrossing.pdfexport` renders the multi-section report and the one-page podium poster with byte-deterministic output across OSes (timezone-aware-UTC creation date, embedded OFL Barlow/Barlow Condensed/DejaVu fonts — D14). The results window is live (E6.4.1): real placed rows from the console's engine source, the tie-break order re-ranks through the control's native arrows, publish checkboxes drive `ExportOptions` (show-times hides the Total column too), a ⚠ badge marks draw-required ties, and `stale_infobar` waits hidden for E7. The Results menu's five export rows, Preview HTML in Browser / Preview PDF in Browser and Tie-break Order are wired (E6.4.2) with off-loop writes (R-02) and the window's own export buttons route through the same handlers; `csvio.export_standings` ships the §15 standings CSV and `csvio.export` gained the §7 finished-ride columns (approved addition). The finish gate (E6.4.3) now runs the evaluator self-test fresh; a red suite blocks finishing with a notice (R-44). **The task-briefs "one task per PR" rule was overridden for this EPIC by product decision: all twelve task commits land in the single EPIC PR (#12), in test-first order.** The known `wx.adv.EditableListBox` seam (generic New/Delete buttons nothing suppresses) is validated-and-restored by the results presenter, matching ride_setup's precedent.
...
Shipped in v0.7. The audited command layer landed on `RideEngine` (E7.1): `edit_crossing`, `void_crossing`, `add_crossing_at`, `reassign_crossing`, `mark_dnf`, and `void_card` — each requiring a non-empty reason and writing exactly one audit event — `undo_last` now carries the fixed label "Undo last crossing", every new action replays through `apply()` (with `Shoe.is_closed` now part of the replay-equivalence contract), and `snapshot()` marks DNF entries so `standings.rank` excludes them (per-rider DNF; E7.1.2's "history + corrections replayed == directly corrected history" property green). The correction UI is live (E7.2): entry detail's six buttons and every §15 Cards-menu route drive the real commands through a new `views/corrections` runner, `DetailPresenter` is implemented, and `ui/menu_state.py` applies `is_route_enabled` at runtime — the missing E1.4.2 live binder. REOPENED is corrections-only (plate entry disabled, corrected crossings bolded in the feed, `reopened_infobar`, single "Finish again" that re-locks and re-ranks). The audit viewer (E7.3.1) is `Store.audit_rows` (newest-first, projected `when/who/action/entry/reason`) plus `AuditPresenter` and `views/audit.py` with plate-or-entry-name search, the six §15 action buckets, and entry-detail deep-linking; the stale-export flag (E7.3.2) marks published results stale after a post-export correction and clears on re-export (watermark = `len(engine.events)`, `None` = never exported). The third mock-first window — `void_card_confirm_dlg` (voids a dealt card) — was authored and its names registered in §15b, retiring the last `_UNAUTHORED_DIALOG` sentinel. **Shoe reopen (product decision):** `finish()` still closes the shoe, but `reopen()` now re-opens it so `deal_manual` and `add_crossing_at` keep dealing in REOPENED, resolving the §4-vs-§15 tension. Doc-silence rulings written back: `reassign_crossing`'s `seq` is the ride-wide ordinal into `engine.crossings`; correction menu routes target the current entry (tracked as `detail_plate` — the dialogs carry no plate/crossing selector); `add_crossing_at` credits directly (never holds); "edited" means a crossing touched by any correction event; `Store.audit_rows` returns the `AuditRow` view-model (the core→presenters seam `demo.py` already uses); the move-rider picker is built in code (no XRC window exists).
