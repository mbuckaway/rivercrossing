...
| R-21 | MUST | CSV import and export per the §7 column spec — one unified, header-mapped format for every plate model (Phase 2; the two old shapes, relay `plate,entry_name,type,rider_1…rider_N` and pooled `plate,name,team_name`, are gone), one row per rider, header resolution and plate/team rules per R-24. Import previews counts + conflicts and writes nothing until confirmed; re-import reshapes teams freely until start. A team-of-one (while DRAFT), a case-insensitive duplicate rider name, and a near-duplicate team name are preview warnings, not blocking conflicts (R-78); duplicate plates and other hard conflicts still block. | §7 · [csvdlg](xrc-windows.md) |
...
| ID | Level | Requirement | Trace |
|---|---|---|---|
| R-79 | MUST | Start gate: a DRAFT ride's Start (button or Ride ▸ Start Ride) refuses until the roster holds at least one entry and the setup clears the minimum-setup rule — non-blank name/venue/organizer/scorer and a positive lap length (`ride.setup_minimum_violations`, surfaced as a `StartBlockedError` notice naming what is missing); Ride Setup refuses the same incomplete submit on `setup_infobar`. The ride's date, planned start and logo are not part of the minimum rule. Starting a stopped ride with data continues it directly — the continue-vs-new prompt and its archive-and-start-fresh path are retired (supersedes R-53's dialog; a fresh ride is File ▸ New Ride…). | §3 · [mainframe](xrc-windows.md)/[setupdlg](xrc-windows.md) |
| R-80 | MUST | A store-backed launch that resumes no ride and deferred no other window shows the no-ride prompt (`no_ride_dlg`) once the console is wired: Create new ride… is the primary (Enter → the ride-setup flow), Open library… the secondary (Escape → the ride library); a launch that already answered the same choice through resume_dlg's Open library never double-prompts. | §2/§3 · [no_ride_dlg](xrc-windows.md) |
| R-81 | MUST | Teams Editor creates teams from its own form — Add team reads the name and notes off the form (no native name prompt), refusing a blank name and any duplicate team name (trimmed, case-insensitive — the same guard applies to a rename on Save) via `teams_infobar`; the new team is created with zero riders and takes the staged logo (card or image) when one was picked. Riders join the team later through the Rider Editor — the anchor-rider mandate of the original wording is amended here (W8), so nothing invents a rider named from the team's own name. | §15/§15b · [team_editor_dlg](xrc-windows.md) |
