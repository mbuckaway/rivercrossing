...
| R-81 | MUST | Teams Editor creates teams from its own form — Add team reads the name and notes off the form (no native name prompt), refusing a blank name and any duplicate team name (trimmed, case-insensitive — the same guard applies to a rename on Save) via `teams_infobar`; the new team is anchored by one rider named from the team's own name and takes the staged logo (card or image) when one was picked. | §15/§15b · [team_editor_dlg](xrc-windows.md) |
