...
| **3 · Functional UI** | The §12 harness drives real wx windows on the runner's desktop. Flake control: event-driven waits (never bare sleeps), two auto-retries per test (`--reruns 2` — the PR #8 Fix-4 bump, adopted after the hosted 3-core runners' churn outran one retry at the full suite size), and a full-screen PNG captured on failure and uploaded as an artifact. |
