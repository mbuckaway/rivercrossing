...
| entry | id · ride_id → · plate (UNIQUE per ride — one namespace with rider plates; a rider_pooled team entry adopts its lowest-numbered rider's plate) · display_name · type (solo \| team) · team_size (2 ≤ n ≤ ride.max_team_size) · status (active \| dnf) · *dnf_at* · *notes* · *logo_card* · *logo_png BLOB* (Phase 4 team logo — card code and/or image bytes; image wins, either clears the other) |
...
- **Elapsed time is wall-clock:** `now − actual_start`. There is no in-memory timer to lose — quit, crash or “stop” never costs time or data. (W6 display decision, R-30: while the ride is stopped the console's clock *display* — elapsed/remaining labels and gauge dials — freezes at the captured value; the engine's wall-clock elapsed keeps running underneath, so Continue unfreezes and jumps the display forward.)

- **Stop** is a UI guard, not a state, and takes three deliberate acts: the console Stop button is disabled until the *Arm* checkbox beside it is ticked, pressing it then opens the **native confirm** (`ui.std_dialogs.show_confirm` — the XRC `stop_confirm_dlg` retired in W5, R-85), and only confirming locks the entry field. The checkbox disarms itself after use or after 10 s untouched. A riderless roster refuses the stop with a native warning before any confirm. Pressing Start on a stopped ride with data continues it directly — `actual_start` unchanged, no prompt (the continue-vs-new prompt retired with R-53); a fresh ride is File ▸ New Ride….
...
- **Radio groups always have a default** — no unselected groups anywhere (defaults: entry mode mixed — `mixed_radio`, "Solo + teams (default)" · plate model rider-pooled · one card per lap · 2 jokers · tie-break ① laps · theme System · Add entry → Solo · manual card → from shoe).
...
**All three formerly windowless §15 routes are now authored mock-first.** Duplicate Ride… and Reopen Ride were authored in E5.4.1 (`duplicate_ride_dlg`, `reopen_ride_dlg` in `dialogs.xrc`). Void Card… — the last 3d-pattern row, whose confirm had no frozen canvas window — was authored in E7 (`void_card_confirm_dlg` in `dialogs.xrc`, control names registered in §15b below before any UI wiring), replacing the E1.4.1 sentinel. No §15 route remains on the `_UNAUTHORED_DIALOG` sentinel.

| Menu item | Opens / does | Design | Enabled when |
|---|---|---|---|
| **File ▸ New Ride…** | Ride setup window (blank) — the single entry point to the setup window since W14 (the duplicate Ride ▸ Ride Setup… row is gone: its "edit this ride" semantics were never built, and both rows always dispatched to this identical New-Ride setup) | 1c | always |
| **File ▸ Ride Library** | Ride library window | 1g | always |
| **File ▸ Duplicate Ride…** | `duplicate_ride_dlg` (E5.4.1, non-destructive confirm — names the ride, Enter-ok) → new DRAFT ride, setup + roster only, no timing data (R-15) | 3d | a ride is open |
...
| **Ride ▸ Reopen Ride** | `reopen_ride_dlg` (E5.4.1, non-destructive confirm — names the ride, Enter-ok) → REOPENED | 3d | FINISHED |
...
| **Riders ▸ Teams Editor** | Teams editor window (Phase 4) — team records: name, relay plate, notes, logo card or image; membership read-only (added in the Rider Editor) | — | ride open, teams enabled (mixed) |
...
- **E5.4.1 mock-first additions — `duplicate_ride_dlg` and `reopen_ride_dlg`:** the two §15 rows (Duplicate Ride…, Reopen Ride) that until E5.4.1 had no frozen window and routed to E1.4.1's `_UNAUTHORED_DIALOG` sentinel (this session's mock-first step, task-brief E5.4.1 plan §2: names registered here and authored in `dialogs.xrc` BEFORE any UI wiring). Each is a confirm dialog naming its ride: `message_lbl` (shared name — the ride's name interpolated, never blank; UX-DESKTOP §4), stock `wxID_OK` + `wxID_CANCEL`. Both are **non-destructive** confirms, so per §13 the primary button carries both `<default>` and `<focused>` and a reflex Enter is safe (unlike the destructive confirms, whose Cancel is default + focused). `duplicate_ride_dlg` confirms File ▸ Duplicate Ride… (copies setup + roster, no timing data — R-15; the copy is a new DRAFT ride whose name the Store derives); `reopen_ride_dlg` confirms Ride ▸ Reopen Ride (FINISHED → REOPENED, spec §3).
...
- **Files (src/rivercrossing/ui/xrc/):** main.xrc → main_frame + main_menubar · setup.xrc → ride_setup_dlg · riders.xrc → rider_editor_dlg + add_rider_dlg (W7) + csv_preview_dlg + rider_issues_dlg (R-78) · teams.xrc → team_editor_dlg (Phase 4) + add_team_dlg (W8) · detail.xrc → entry_detail_dlg · results.xrc → results_frame · library.xrc → ride_library_dlg + delete_ride_dlg · audit.xrc → audit_dlg · settings.xrc → settings_dlg · dialogs.xrc → set_start_dlg, finish_confirm_dlg, resume_dlg, exit_running_dlg, exit_confirm_dlg, duplicate_ride_dlg, reopen_ride_dlg, edit_crossing_dlg, reassign_dlg, manual_deal_dlg, dnf_confirm_dlg, void_card_confirm_dlg, about_dlg, selftest_dlg, shortcuts_dlg — `stop_confirm_dlg` retired in W5 (native confirm, R-85) and `no_ride_dlg` retired in W15 (native info alert, R-80/R-85).

- **Menu item names** (map 1:1 to §15 rows): File — mi_new_ride, mi_open_library, mi_duplicate_ride, mi_backup_now, mi_import_csv, mi_export_csv, wxID_PREFERENCES (Settings…), wxID_EXIT · Ride — mi_start_ride, mi_stop_ride, mi_set_start_time, mi_finish_ride, mi_reopen_ride, mi_audit_trail (W14: `mi_ride_setup` left with the removed Ride Setup row — File ▸ New Ride… is the single entry point to the ride-setup window) · Riders — mi_rider_editor, mi_team_editor (Phase 4), mi_check_rider_issues (R-78), mi_add_entry, mi_mark_dnf, mi_entry_detail · Cards — mi_undo_crossing, mi_add_crossing_at, mi_edit_crossing, mi_reassign_plate, mi_deal_manual, mi_void_card, mi_review_held · Results — mi_standings (F5), mi_export_html, mi_export_pdf, mi_export_poster, mi_export_results_csv, mi_preview_browser, mi_tiebreak_order · View — mi_hide_times (check item), mi_zoom_90…mi_zoom_150 (radio items) — the three `mi_theme_*` radios left with W13, since the Settings appearance radios are the single theme surface · Help — mi_user_guide, mi_shortcuts, mi_selftest, wxID_ABOUT.
