...
| entry | id · ride_id → · plate (UNIQUE per ride — one namespace with rider plates; a rider_pooled team entry adopts its lowest-numbered rider's plate) · display_name · type (solo \| team) · team_size (2 ≤ n ≤ ride.max_team_size) · status (active \| dnf) · *dnf_at* · *notes* |
...
- **Rider-pooled rides use one row per rider instead:** `plate,name,team_name,notes` — riders sharing a team_name form a team (blank = solo). Re-imports may regroup teams; while RUNNING a changed team_name is treated as a membership move (audit-logged), not a conflict. `team_name` is the merge key, so it is unique per file by construction — two intended teams must not share one name. A team's `notes` are its members' row notes joined with "; " on import; export writes the team's notes on its first member row only. A one-rider group is reported as *team-under-min*: R-12's floor is enforced at preview/commit and at ride start, which lets the editor assemble a team one rider at a time in DRAFT.

- **Import semantics:** match on plate (update in place), insert new plates, and list every conflict before anything is written — preview first, commit second, nothing touched on preview. In DRAFT a re-import applies every reshape the editor can make, including converting solo ⇄ team and adding brand-new riders onto existing teams; while RUNNING, rider-pooled keeps team-to-team moves and brand-new plates joining a team (both audited) and conversions wait for DRAFT.
