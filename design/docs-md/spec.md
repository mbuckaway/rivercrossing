...
**One route still has no frozen window.** Void Card… cites the "3d pattern" — a confirm dialog that exists only in the retired hi-fi designs, so no window in the XRC canvas covers it. The gap is owned by **EPIC 7** (Void Card…), authored mock-first with its control names registered in §15b before any UI code; EPIC 1 routes it to a flagged sentinel the menu-coverage test asserts as such — never an invented window name. The other two 3d-pattern rows, Duplicate Ride… and Reopen Ride, were authored mock-first in E5.4.1: `duplicate_ride_dlg` and `reopen_ride_dlg` in `dialogs.xrc`, their control names registered in §15b below before any UI wiring, replacing the E1.4.1 sentinel.
