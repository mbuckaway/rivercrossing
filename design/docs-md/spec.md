...
- **Declared in XRC vs code:** XRC owns structure, labels, sizers, styles (wxRB_GROUP radio groups inside wxStaticBoxSizer, wxStdDialogButtonSizer button rows, wxSplitterWindow, wxDataViewCtrl shells). Code owns: DataView columns/rows/attributes, card imagelist (53 bitmaps @1x/2x from ui/assets/cards/), InfoBar construction + messages + show/hide, splitter sash restore, window minimum sizes, radio menu-item defaults, per-state menu enabling, SetAppearance; plus (W10) the ride library's macOS-26 `.glass` button bezel — a native-bezel selection applied through `GetHandle()`→`setBezelStyle:` (§15b), not restyling. Custom-drawn widgets: none.

- **Three classes cannot be authored in XRC at all** (measured on 4.3.1 / wxWidgets 3.3.3): **wxInfoBar** — the handler yields a generic `wx.Control`, not a `wx.InfoBar`, and silently drops `name`, so the eleven code-side info bars (`resume_infobar`, `reopened_infobar`, `finished_infobar` in main_frame, `stale_infobar` in results_frame, `setup_infobar` in ride_setup_dlg, E3's `roster_infobar` in rider_editor_dlg and `csv_infobar` in csv_preview_dlg, Phase 4's `teams_infobar` in team_editor_dlg, R-78's `issues_infobar` in rider_issues_dlg, W7's `add_rider_infobar` in add_rider_dlg and W8's `add_team_infobar` in add_team_dlg) are built with `wx.InfoBar()` in code and given their frozen names with `SetName()` — W11's `finished_infobar` carries two `AddButton` children (`finished_reopen_btn`, `finished_results_btn`), real `wx.Button`s named code-side the same way, since `AddButton` creates the buttons at runtime and XRC cannot name them either — each with its slide effects disabled via `SetShowHideEffects`, since the default effect hangs `ShowMessage()`/`Dismiss()` on this baseline (measured in E3) · **wxDataViewListCtrl** — its handler hard-forces the control name to `dataviewCtrl`, so no frozen name resolves; all twelve list controls are therefore authored as **wxDataViewCtrl**, whose name *is* honoured, and per-row attributes come from a `DataViewIndexListModel` subclass overriding `GetAttrByRow` (there is no setter — `SetAttrByRow` and `SetItemAttr` do not exist) · **wxMenuBar** — also drops its name: `main_menubar` is the resource id `XmlResource.LoadMenuBar()` loads by, and it never resolves through `FindWindowByName`.
