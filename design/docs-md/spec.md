...
- **Exit is caught:** closing the app (window × or File ▸ Exit) with a ride RUNNING opens the exit dialog — Cancel · Finish ride first… · Quit-keep-running. Quitting writes `app_session.closed_at` and leaves the ride RUNNING; the wall clock keeps counting while the app is closed.
...
| Event | Behaviour |
|---|---|
| App closed (× / Exit) while running | Exit dialog: Cancel · Finish first · Quit-keep-running. Ride stays RUNNING on wall clock; relaunch → resume dialog ("you quit at…"). |
...
- **Installers:** PyInstaller builds → Windows: Inno Setup (.exe installer, per-user, unsigned — see §14) · macOS: .app in a .dmg via dmgbuild, Developer ID codesigned + notarized. CI matrix builds both per tag (§14). Bundles include the third-party license texts shown by About ▸ Licenses… — a redistribution obligation for the OSS dependencies.
...
| **6 · Package (tags)** | Windows: Inno Setup .exe, **unsigned** (no Authenticode cert available — the guide documents the SmartScreen "More info → Run anyway" step) · macOS: dmgbuild → Developer ID codesign + notarytool staple (Apple developer account available). Installers uploaded as release artifacts with checksums. |

Every PR runs stages 1–4; main additionally runs 5; version tags run all six and draft the release. Stage 5 runs in **dev-bundle mode from EPIC 1 (D1) onward** — PyInstaller onedir, unsigned, uploaded as runnable Windows + macOS artifacts on every main build — and graduates to full installers in EPIC 9. A nightly job replays the acceptance race with fresh random seeds and files the seed on failure so any hand-evaluation edge case is reproducible.
...
| **File ▸ Exit** | Ride RUNNING → exit dialog (Cancel · Finish first · Quit-keep-running); otherwise quits. Reopen → resume dialog | 4a · 1h | always |
