...
⚠ code-side: entry/plate-model group locks after start (relay) or stays editable (pooled, R-17); tiebreak_list reorder persisted; decks_spin's value — Spec §4 says 8 decks by default, the canvas draws 2, and the number is an **open question for the ride-setup work (E3/E4)**: the XRC declares no value and the presenter supplies it. All fields plain XRC.
...
⚠ code-side: standings rows; "draw required" tie rows highlighted with a ⚠ badge column; stale-export flag banner (wxInfoBar stale_infobar — built in code, named with SetName()) after reopened corrections.
