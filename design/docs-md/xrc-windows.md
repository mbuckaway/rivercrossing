...
Edit Crossing / Add Crossing at Time / Edit Time`edit_crossing_dlg`✕

Plate
Time
Reason

`plate_input · time_picker · reason_input · void_btn (menu Edit Crossing only — hidden, with plate_input read-only, for Crossing Detail's Edit Time)`

Void crossing…OKCancel

⚠ one XRC dialog, three titles: Cards ▸ Edit Crossing… sets "Edit Crossing", Cards ▸ Add Crossing at Time… sets "Add Crossing at Time", and Crossing Detail's Edit Time… sets "Edit Time", all three in code with the prefill; reason required, audit-logged. Crossing Detail's Edit Time opens it with a read-only plate and no `void_btn`, while the two menu paths keep `void_btn` and an editable plate.
