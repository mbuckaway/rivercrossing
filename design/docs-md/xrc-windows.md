...
- **Canvas caveats (HTML approximation):** browser form controls stand in for wx natives; exact spacing/fonts come from sizers + system fonts, not these pixels; sizes below are minimums expressed in dialog units at build time. **Global code-side items (not expressible in XRC):** ① DataView columns + row data + per-row attributes — appended in code: the **bold** flagged/edited rows through a `DataViewIndexListModel` subclass overriding `GetAttrByRow` (since no setter exists), and the **card cells' red suits** through wx **markup** — a `wx.dataview.DataViewTextRenderer` with `EnableMarkup()`, built into a `DataViewColumn` and appended with `AppendColumn` (`AppendTextColumn` takes no renderer), because a `DataViewItemAttr` carries one colour per *cell* and so cannot colour one card inside a mixed-suit five-card "Best 5" hand or the console Riders Cards cell's whole hand; only hearts and diamonds carry a colour of their own (`#c0392b` in a light appearance, `#e57373` in a dark one, decided by `SystemSettings.GetAppearance().IsDark()`), while spades, clubs and the joker draw no colour and inherit the list's own foreground, so they follow the appearance like every other cell — no high-contrast probe exists in this wx build, so those two red suits keep their fixed red where every other cell would honour an OS high-contrast theme; ② card imagelist population (53 card bitmaps @1x/2x); ③ wxInfoBar **construction** + message text + Show/Hide calls, and the console's two custom gauges — the `RaceClock` dials (`elapsed_clock`/`remaining_clock`) and the `StopLight` (`ride_status_light`) — built code-side into main.xrc's `elapsed_clock_panel`/`remaining_clock_panel`/`ride_status_panel` placeholder slots and named with `SetName()`, because XRC cannot author a `wx.Control` subclass (ux-polish); ④ splitter sash position restore from settings; ⑤ menu enable/disable per ride state (§15); ⑥ theme: `wx.App.SetAppearance` on the 4.3.1 / wxWidgets 3.3.3 baseline — all three appearance radios live on both platforms (measured: macOS applies at runtime to existing windows; `Appearance::System` pins the NSAppearance current at the call instead of restoring follow-the-system, so the app re-applies System on `wx.EVT_SYS_COLOUR_CHANGED`, best-effort; MSW on 3.3.3 returns `CannotChange` once a top-level window exists, so a Windows theme change takes effect at next launch and the status bar says so); ⑦ window minimum sizes via `SetMinSize()` (XRC has no window-level minsize); ⑧ the zoom menu-item default `mi_zoom_100` (`<checked>` is a no-op on radio items; W13 removed the View-menu theme trio — the Settings appearance radios are the single theme surface, so the old `mi_theme_system` default went with them). Everything else drawn here is declared in XRC.
...
⚠ The console carries no `wxInfoBar` in any state (G4): the resume prompt is the modal `resume_dlg` (section B), and the REOPENED corrections notice is the status bar's first field — nothing sits above the ride-info block.
...
`crossings_list (wxDataViewCtrl · every crossing of the ride — no cap, the list scrolls (Phase 4 retired R-32's 30-row window) — every column sortable and resizable through CrossingsFeedModel.Compare (both flags spelled out, since an explicit flags= replaces AppendTextColumn's own default; the platform's own header arrow), opening on the Time column descending, so the newest crossing is the first row (the operator's reading order). Columns in the W9 order Time · Plate · Name · Team · Card · Lap · Lap time · Total with one pinned width per column (80, 50, 150, 130, 60, 50, 80, 80 — DataView has no autosize-to-content, so the widths are data), the Total and Lap time columns R-37's two independent show settings each hide. The Time cell is the *elapsed* reading since the gun (h:mm:ss from 0), never the wall-clock crossing instant, and Total is the entry's own running sum; a DNF rider's Name cell carries a " DNF" suffix. The Team cell reads `solo` for a solo entry — never blank, and never the rider's own name repeated from the Name column (blank only for a crossing whose plate no longer resolves) — the same word the rider lists show. The Card column carries the real dealt code — a held crossing's row shows the held card's own code, never a placeholder — except a voided card, whose cell reads `Void`; the row renders bold while its card is held for review or the row is one half of a live duplicate pair (R-34), so a resolved (confirmed or voided) short lap renders at regular weight while still listing in Needs Review`
...
Riders   Needs Review — `review_notebook (wxNotebook · two tabs — ux-polish; the notebookpage node's own name is dropped by the XRC handler, so the names live on the page children below)`
...
Issue | Card | Plate | Lap | Lap time | Rider   `flagged_list (wxDataViewCtrl — the R-34 flag rows, six columns at widths 136/76/64/52/48/256; the Card column reads Held/Credited/Void via feed_model.card_status_text, the Issue column reads Short lap or Duplicate crossing, Lap time reads FeedRow.lap_time and Rider reads FeedRow.entry)`
...
⚠ Retirement: the finish question now asks through the native danger confirm (`ui.std_dialogs.show_danger`, R-85) — the retired window's own line, kept in `views.dialogs.finish_ride_message` (E6.4.3 reworded its parenthetical): "Locks entry and computes final standings (a blocking evaluator self-test failure asks for a second confirm and marks the results self-test unverified). You can reopen later for corrections." — with Finish ride / Cancel (Cancel default + focused). Ride ▸ Finish Ride… shows it; from REOPENED the same confirm is retitled "Finish again?" with the primary button "Finish again" (`finish_again_labels`). E6.4.3: once the first confirm is accepted the route runs the evaluator self-test itself (`console.FINISH_GATE`) and, when a BLOCKING check failed, asks one more native danger confirm — title "Evaluator Self-Test Failed", primary button "Finish anyway" (`views.dialogs.self_test_override_labels`), its message naming every failed check and stating the consequence (`self_test_override_message`) — so a red evaluator never strands a multi-hour ride and is never overridden silently; Cancel there leaves the ride unfinished and still finishable. Finishing anyway writes the failed check names onto the finish event (`self_test_failed_checks`), and the ride's results then read "Self-test unverified" until a clean re-finish. (The parenthetical names E6.4.3's behaviour: the advisory 180×12 timing check never gates a finish, and a blocking failure asks a second confirm.) On OK the status lamp goes red and the status bar reads "Ride is finished. Results are available"; the finish-time auto-export of HTML and PDF into the per-user `exports` folder was removed, so the operator exports on demand from the Results menu (Results ▸ Export HTML/PDF/Podium poster/CSV) or the results dialog's own export buttons (E2). The window left dialogs.xrc and the frozen-name registry in H2.
...
`message_lbl (ride name + quit/crash wording interpolated) · continue_btn (a custom id, never wxID_OK — the button keeps its name and the XRC marks it `default`/`focused`; the affirmative behaviour is wired code-side with `SetAffirmativeId(continue_btn.GetId())`, so Enter returns the button's own id) · library_btn`
...
Rider — PlateFirst nameLast nameTeamTrail Blazers

Edit Rider…

`plate_input · first_name_input · last_name_input (all wxTE_READONLY — 1.0.12 B1: the box is a read-only display of the selected record, never typed into, so the plate lock and the dirty-form save gate both left with the editable form) · team_choice (a read-only wxTextCtrl, `wxTE_READONLY` in the XRC like the three fields above — Phase 3 made the editor's Team field a read-only display of the selected record's team ("— solo —" for a solo rider), never a dropdown: team assignment is the Add/Edit dialog's job) · edit_btn ("Edit Rider…" — 1.0.12 B2: opens add_rider_dlg on the selected record, and a row double-click or Enter is the same route; 1.0.13: it alone anchors the form pane's bottom-right, acting on the record shown above it, so it carries a plain custom id — never wxID_OK — and wx never auto-closes on a click)`
...
`plate_input · time_picker · reason_input · void_btn (shown only for the dialog's own `adding=False` default; hidden for the Add Crossing route, and suppressed for Crossing Detail's Edit Time, whose plate_input is read-only)`

Void crossing…OKCancel

⚠ one XRC dialog, two reachable titles: Cards ▸ Add Crossing at Time… sets "Add Crossing at Time", and Crossing Detail's Edit Time… sets "Edit Time", both in code with the prefill; reason required, audit-logged. The retired Cards ▸ Edit Crossing… row set "Edit Crossing" — the dialog's own `adding=False` default, which no route reaches now (1.0.17 follow-up). Crossing Detail's Edit Time opens it with a read-only plate and `void_btn` suppressed, and the Add Crossing path hides `void_btn` too (`adding=True`) while offering an editable plate — so `void_btn` is shown only in that unreachable `adding=False` mode.
...
Standings — GORBA EPIC 2026 (FINISHED 16:02:11)`results_dlg`✕
...
| Place | Plate | Entry | Laps | Best 5 | Hand | Draw |
|---|---|---|---|---|---|---|
...
⚠ code-side: standings rows — the presenter feeds each list its own kind (the notebook's two pages on a MIXED ride, the lone standings_list on a SOLO one, Phase 3, R-65); "draw required" tie rows highlighted with a ⚠ badge in the Place cell (the seven pinned columns), the badge explained on activation — double-click or Enter opens an OK-only `std_dialogs.show_info` alert carrying the row's own tie note ("draw required" plus the sentence explaining that the configured tie-break order could not separate the hands and the venue decides: `results_win.DRAW_EXPLANATION`), since a DataViewCtrl has no per-row tooltip and hover is keyboard-unreachable; the Draw column beside Hand shows the card an entry drew for the venue's high-card tie-break (`StandingsRow.tiebreak_card`, blank when it drew none) — a drawn tie is *resolved*, so only the residual tie carries the badge; one code-side InfoBar (`stale_infobar` — built in code, named with SetName()) carries the dialog's two notes, the stale-export flag after reopened corrections (`results_win.STALE_NOTE`) taking precedence over E6.4.3's "Self-test unverified" note (`results_win.SELF_TEST_NOTE`) for a ride the operator finished over a failed evaluator self-test. G6 moved the five publish options out of this dialog onto checkable Results-menu items (each persisting in `AppSettings.publish_*`), so the dialog carries no checkbox and the show-times setting no longer hides any column here. The window is a modal wxDialog opened through dialogs.run_dialog — a transient over the console, so its title bar carries the ✕ alone (no ▢), and the tie-break panel and Reopen ride button left with the frame: tie-break order is ride_setup_dlg's own tiebreak_list and nothing else, and reopening a ride is the menu's Ride ▸ Reopen Ride (mi_reopen_ride). Standings is always available — the Results ▸ Standings row (F5) is never gated on a ride open; with none open the dialog renders its empty state. The Teams list hides its Plate column under `rider_pooled` (a pooled team's plate is derived from its members, so the column would only repeat one) and keeps it under `team_relay`, where the plate is the entry's identity; the solo list keeps it always. It floors at 800 px wide (MIN_SIZE = 800×442, code-side: the seven pinned columns total 750 px plus the list's scrollbar and the notebook/sizer borders — the publish-checkbox row that used to set the 755 px floor is gone), with each of the three standings lists (`standings_list`/`teams_standings_list`/`solo_standings_list`) floored at 198 px high — a 28 px header plus 10 × 17 px rows (measured on wxPython 4.3.1), so ten standings rows are visible. The window lives in results.xrc (§15b). Phase 7 adds the DNS row: a FINISHED ride's 0-lap ACTIVE entry is appended, unplaced, at the bottom of its own section (Teams or Solo) with a blank Place cell, the literal `DNS` in the Laps cell and a blank Draw cell — no tie-break badge, the entry having never joined the ranking — and the Results menu's Show DNS Riders toggle (`mi_show_dns_riders`, on by default) shows or hides those rows; a live or REOPENED ride ranks its whole field, so it renders none.
...
⚠ code-side (E6.4.3): the six checks split into five BLOCKING correctness checks and one ADVISORY check — the whole-field 180×12 timing check (`hands._check_field_timing` runs with `blocking=False`), which measures this host's speed, not the evaluator's correctness. The dialog's own verdict (`SelfTestReport.passed`) still shows every red line as FAIL, advisory included, so the canvas above is unchanged; the finish gate reads `SelfTestReport.has_blocking_failure` instead, so the timing check can never strand a ride and only a blocking failure raises the app's override confirm (Ride ▸ Finish Ride…, its ⚠ Retirement note above) naming `report.failed_blocking_checks`.
