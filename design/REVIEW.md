...
| XRC naming | 113 named controls across the 23 windows (111 annotated on the canvas, plus `stale_infobar` and `main_splitter`, which appear only in its code-side footnotes; the 1.0.17 follow-up retired the annotated `edit_plate_input` and the code-side `finished_infobar`), all suffix-conventional; no duplicates within a window, and names repeat *across* windows by design (§15b requires uniqueness only within one). The authoritative count is whatever `tools/gen_ids.py` extracts from the .xrc files — 171 constants once the menu items and window ids are counted — held honest by the drift gate (R-05). |
