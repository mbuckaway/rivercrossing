...
The test-only fixture's own ``cards_held`` only carries 5 of the
entry's 9 dealt cards -- the rest are illegible in the canvas and are
not invented there; ``show_entry`` renders exactly what the source
gives it. E5.4.2 wired the app's entry-detail route to
``EmptyDataSource`` (no store-backed entry selected yet, E7 wires the
real lookup), so the dialog opens with an empty
header/members/cards/laps until then.
