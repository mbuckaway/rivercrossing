...
results page and POSTs it to the site. The four decisions the dialog
makes are all UI-side and live here, not in the app: what each field
opens with, which status labels the choice offers, why a publish is
refused before the site is ever contacted, and that a refused publish
leaves the dialog open for a correction.
...
(``STATUS_CHOICES``, ``default_title``, ``default_slug``,
``status_value``, and :meth:`PublishForm.errors`), so the rules are
unit-tested without a display and this module only wires widgets.
...
# The index wp_status_choice opens on for a stored status. A stored
# value this build does not offer (an older or hand-edited settings
# file) falls back to the first item -- Draft, the presenter's own
# _DEFAULT_STATUS -- so the choice is never left unselected.
_DEFAULT_STATUS_INDEX = 0
...
class PublishWordpressDialog(DialogFindMixin):  # _find: ui.views._support
        ...
        *,
        ...
        Args:
            ...
            settings: The live :class:`AppSettings` the site fields
                ...
                ``wp_password``/``wp_parent``/``wp_status``, so the
                operator re-publishes an already-configured site
                without retyping anything, and the password never has
                to be re-entered.
    ...
    def show_form(self, settings: AppSettings, ride_name: str) -> None:
        ...
        this ride's page, not the previous one's. ``wp_status_choice``
        gets its items here rather than in XRC: the labels and their
        WordPress values come from one place
        (:data:`STATUS_CHOICES`), so a label can never render beside
        another value's name.
        ...
        self.title_input.SetValue(default_title(ride_name))
        self.slug_input.SetValue(default_slug(ride_name))
        self.parent_input.SetValue(settings.wp_parent)
        # Set() clears any selection, so the index is set after it.
    ...
    def collect_form(self) -> PublishForm:
        ...
        a whitespace-only box as empty rather than as input. The status
        reads through :func:`status_value`, so a label this build does
        not know can never publish something by accident.
        """
        return PublishForm(
