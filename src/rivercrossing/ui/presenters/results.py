...
"""Results presenter -- results_dlg (1f), standings and staleness.
...
teams/solo split. G6 retired the publish-options channel it used to
hold (``export_options``/``on_publish_toggled``): the export flags now
live in ``AppSettings.publish_*``, read straight from the settings by
``app._export_options``, so the presenter keeps only the standings and
the stale-export flag.
...
class ResultsPresenter:
    ...
    Ranks the standings with the ride's stored tie-break order.
    E7.3.2 adds the stale-export flag: the presenter holds the export
    watermark and drives :meth:`ResultsView.set_stale` on the first
    render.
    """
